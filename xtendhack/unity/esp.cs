...
namespace xtendhack
{
  partial class hack : MonoBehaviour
  {
    ...
    private void esp(Vector3 entity_position, Color color)
    {
      ...
      float box_thickness = 2f;

      if (entity_screen_pos.x > 0 && entity_screen_pos.x < Screen.width && entity_screen_pos.y > 0 && entity_screen_pos.y < Screen.height)
      {
        render.draw_box_outline(new Vector2(entity_screen_pos.x - box_width / 2, entity_screen_pos.y - box_height / 2), box_width, box_height, color, box_thickness);
        render.draw_line(new Vector2(Screen.width / 2, Screen.height), new Vector2(entity_screen_pos.x, entity_screen_pos.y + box_height / 2), color, 2f);
        render.draw_string(new Vector2(entity_screen_pos.x, entity_screen_pos.y - 20 + box_height / 2), "Object", color, true);
      }
    }
  }
}